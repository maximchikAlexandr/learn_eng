...
class WordRepository:
    def __init__(self, session_, eng_word_dct):
        ...
        self.__eng_word_model = EngWord(
            eng_word=eng_word_dct["text"], ts=eng_word_dct["ts"]
        )
        ...
        for pos, examples in self.__eng_word_dct["ex"].items():
            for ex in examples:
                example_model = Example(
                    example=ex,
                    id_pos=PartOfSpeech.query.filter(PartOfSpeech.pos == pos)
                    .first()
                    .id,
        ...
        for pos, tr_lst in self.__eng_word_dct["tr"].items():
            for tr in tr_lst:
                if not TrRusWord.query.filter(TrRusWord.rus_word == tr).all():
                    ...
                    tr_rus_word = TrRusWord.query.filter(
                        TrRusWord.rus_word == tr
                    ).first()
        ...
        if not self._is_existing_text():
            for word in get_unic_words(self.__text):
                if WordRepository.is_existing_word(word):
                    eng_word_model = EngWord.query.filter(
...
class Pagination:
    ...
    def pagination_for_texts(id_user, page, per_page):
        print(
            "text",
            Text.query.filter(Text.id_user == id_user).paginate(
                page=page, per_page=per_page
            ),
        )
        return Text.query.filter(Text.id_user == id_user).paginate(
            page=page, per_page=per_page
        )
    ...
    def pagination_for_words(id_text, page, per_page):
        print(
            "word",
            Text.query.filter(Text.id == id_text)
            .one()
            .words.paginate(page=page, per_page=per_page),
        )
        return (
            Text.query.filter(Text.id == id_text)
            .one()
        ...
        res = [
            {
                ...
                "tr": [
                    {
                        pos.pos: [
                            tr.rus_word for tr in wrd.translated_words if pos == tr.pos
                        ]
                    }
                    for pos in set(tr.pos for tr in wrd.translated_words)
                ],
                "ex": [
                    {pos.pos: [ex.example for ex in wrd.examples if pos == ex.pos]}
                    for pos in set(ex.pos for ex in wrd.examples)
                ],
            }
            for wrd in words
        ]
