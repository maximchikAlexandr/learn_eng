...
from dct import db
from dct.repository import Pagination, TextRepository
...
def index():
    ...
    pages = Pagination.pagination_for_texts(
        id_user=current_user.get_id(), page=page, per_page=PER_PAGE_TEXTS
    )

    dates = {txt.id: sec_to_datetime(txt.time) for txt in pages.items}

    return render_template("dct/index.html", title="Texts", pages=pages, dates=dates)
...
def add_text():
    if request.method == "POST":
        if request.form["text"] and request.form["title"]:
            text = TextRepository(
                db.session, text=request.form["text"], title=request.form["title"]
            )
            text.save()
...
def show_text(id_text):
    page = request.args.get("page")
    page = int(page) if page and page.isdigit() else 1
    pages = Pagination.pagination_for_words(
        id_text=id_text, page=page, per_page=PER_PAGE_WORDS
    )

    words = Pagination.get_words_from_pagination(pagination=pages)
    current_text = TextRepository.get_text(id_text=id_text)
    return render_template(
        "dct/show_text.html", words=words, text=current_text, pages=pages, title="Text"
    )
