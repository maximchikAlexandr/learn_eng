def _searching_error(username: str, email: str, psw: str, psw2: str) -> dict:
    ...
    digit_error = (
        len([ch for ch in psw if str(ch) in "0123456789"]) < 3
    ...
    unique_symbols_error = (
        len(set(psw)) / len(psw) < 0.75
    )  # check count of a unique symbols
...
def check_data_user(request) -> dict:
    ...
    return {
        ...
        "flashed_messages": [
            flashed_messages[key] for key, value in check_dct.items() if value
        ],
    }
