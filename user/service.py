def _searching_error(username: str, email: str, psw: str, psw2: str) -> dict:
    ...
    min_len_name_error = len(username) > 6
    ...
    ascii_error = psw.isascii()  #  searching for ascii
...
def check_data_user(request)  -> dict:
    ...
    flashed_messages = {
        ...
        'min_len_name_error': 'Min length of username is 6 symbols',
    ...
    return {
        'password_ok' : all(check_dct.values()),
        'flashed_messages' : [flashed_messages[key] for key, value in check_dct.items() if not value]
    }
